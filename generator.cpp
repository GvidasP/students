...
std::string writeStudents(int amount, std::string students_filename="studentai")
{
    auto start = std::chrono::steady_clock::now();

    std::vector<Student> students = generateStudents(amount);

    std::string file_name_1 = students_filename + "_kietekai" + std::to_string(students.size() / 2) + ".txt";
    std::ofstream students_file_1(file_name_1);

    std::string file_name_2 = students_filename + "_vargsiukai" + std::to_string(students.size() / 2) + ".txt";
    std::ofstream students_file_2(file_name_2);

    std::string file_name = students_filename + std::to_string(students.size()) + ".txt";
    ...
    for (Student& s : students)
    {
        ...
        if(s.getCategory().compare("vargsiukas")) {
            students_file_2 << s;
        }
        else
        {
...
void writeInputStudents(std::vector<Student> students, std::string students_filename)
{
    std::string file_name = students_filename + std::to_string(students.size()) + ".txt";
    std::ofstream students_file(file_name);
